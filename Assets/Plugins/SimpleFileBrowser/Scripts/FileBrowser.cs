...
namespace GracesGames.SimpleFileBrowser.Scripts
{
	...
	public class FileBrowser : MonoBehaviour
	{
		...
		// Строка используемая для фильтрации файлов по имени 
		...
		public void SetupFileBrowser(ViewMode newViewMode, string startPath = "")
		{
			...
			if (ViewMode == ViewMode.Portrait)
				userIterfacePrefab = FileBrowserPortraitUiPrefab;
			else
				userIterfacePrefab = FileBrowserLandscapeUiPrefab;
		...
		public void MySetupFileBrowser(ViewMode newViewMode, RectTransform parent, string startPath = "")
		{
			ViewMode = newViewMode;
			...
			GameObject fileBrowserUi = Instantiate(userIterfacePrefab, parent.transform, false);
			fileBrowserUi.transform.SetParent(parent);
			fileBrowserUi.transform.localPosition = Vector3.zero;
		...
		private bool IsTopLevelReached()
		{
			...
			return _currentPath == _rootDirectory;
		}
		...
		private void BuildFiles()
		{
			...
			LayoutRebuilder.ForceRebuildLayoutImmediate(_uiScript.DirectoriesParent.GetComponent<RectTransform>());
		}
