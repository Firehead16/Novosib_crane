...
namespace Core.Ui.FancyScrollView.Examples.FancyScrollViewExample04
{
    class Background : MonoBehaviour, IPointerClickHandler
    {
        ...
        {
            float LocalMethod(Vector2 v) => 1f / (v.x * v.x + v.y * v.y + 0.0001f);

            const float scale = 4600f;
            var d = cellState.Sum(x => LocalMethod(p - new Vector2(x.x, x.y)) * x.w);
