...
public class WizardReplaceSplatmap : ScriptableWizard
{
	...
	void OnWizardCreate()
	{
		...
		try
		{
			...
			if (extract)
			{
				Texture2D texture1 = new Texture2D(newsplat.width, newsplat.height, TextureFormat.ARGB32, false);

				for (var i = 0; i < pixels.Length; i++)
				{
					var tempPixel = pixels[i];
					tempPixel = new Color(tempPixel.r, tempPixel.g, tempPixel.b, 1);
					pixels[i] = tempPixel;
				}
				...
				SaveTextureAsPng(texture1, Application.dataPath +"/Out.png");
			}
		}
		catch (UnityException)
		{
			EditorUtility.DisplayDialog("Not readable",
				"The 'New' splatmap must be readable. Make sure the type is Advanced and enable read/write and try again!",
				"Cancel");
			return;
		}
