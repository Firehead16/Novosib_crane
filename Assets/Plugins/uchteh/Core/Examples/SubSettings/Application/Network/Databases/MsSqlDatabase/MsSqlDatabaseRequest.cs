...
public sealed class MsSqlDatabaseRequest : BaseControlMethods, IAuthorizationDatabaseRequest, ITestDatabaseRequest, IHostRequest
{
	...
	{
		int id = Database.SendExecuteScalarWithLastId($"INSERT INTO TestQuestions (Name, TestId, Type, Advice) VALUES ('{question.Name}', '{question.TestId}', '{(int)question.TypeQuestion}','{question.Advice}'); ");
		return id;
	}
	...
	public List<AnswerDragNDrop> GetDragNDropList(int pQuestionId)
	{
		...
		using (SqlConnection connection = new SqlConnection(Database.ConnectionString))
		{
			connection.Open();
			SqlCommand cmd = connection.CreateCommand();
			SqlTransaction checkTransaction = connection.BeginTransaction();
